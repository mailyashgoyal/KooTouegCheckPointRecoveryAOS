...
public class PendingMessageQueue {
	...
	public void addTimeStampReply(Message m, int remoteId){
		...
		int proposedTimeStamp = m.getLabel();
		...
		for (PendingMessage pm : queue) {
			...
			if (pm.messageId == msgId) {
				if (m.getType().equals(MessageType.APPLICATION)) {
					pm.updateProposedTimeStamp(proposedTimeStamp);
					pm.addReplyCount();
				}

				if (m.getType().equals(MessageType.CHECKPOINT)) {
				...
				if (m.getType().equals(MessageType.CHECKPOINT_REPLY)) {
					pm.updateProposedTimeStamp(proposedTimeStamp);
					pm.addReplyCount();
				}
				if (m.getType().equals(MessageType.CONTROL)) {
					pm.updateProposedTimeStamp(proposedTimeStamp);
					pm.addReplyCount();
				}
				if (m.getType().equals(MessageType.RECOVERY)) {
					pm.updateProposedTimeStamp(proposedTimeStamp);
					pm.addReplyCount();
				}
